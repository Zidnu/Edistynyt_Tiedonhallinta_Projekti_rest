...
# Tähän jouduin hieman kyselemään apua tekoälyltä ja yritin netistä kans kattoa hieman mallia.
# Jumituin tähän alun toteutukseen

# Yhden päivän mittaukset
@app.route('/measurements/<int:loc_id>', methods=['GET'])
def get_daily_measurements(loc_id):
    # Päivän haku
    date = request.args.get('date')
    result = open_aq_database.get_location_daily_measurements(loc_id, date)
...
def get_total_measurement(loc_id):
    ...
    result = open_aq_database.get_total_place_measurement(loc_id)
    return jsonify(result)
...
def get_daily_average(loc_id):
    ...
    date = request.args.get('date')
    result = open_aq_database.get_daily_avg_for_sensor(loc_id, sensor_id, date)
    return jsonify(result)
